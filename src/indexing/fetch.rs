...
mod test {
    ...
    use crate::{
        indexing::fetch::{cache_remote_objects_inv, fetch_objects_inv_blocking},
        parsing::sphinx::inv_file::parse_objects_inv_file,
    };

    // ignored until I figure out how I want to segreate tests that reqquire outside interactions like
    // this fetch test
    #[ignore]
    #[test]
    fn fetch_numpy_obj_inv() -> Result<()> {
        let url = "https://numpy.org/doc/stable/objects.inv";

        fetch_objects_inv_blocking(url)?;

        let tmp_path = PathBuf::from("tmp_obj.inv");

        let _ = parse_objects_inv_file(&tmp_path)?;

        Ok(())
    }

    #[test]
    fn cache_numpy_obj_inv() -> Result<()> {
        let url = "https://numpy.org/doc/stable/objects.inv";
        cache_remote_objects_inv(url, "numpy".to_string(), None)?;
        Ok(())
    }
    #[ignore]
    #[test]
